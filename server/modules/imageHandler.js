...
// uploads photo for client profile
...
// VT registration post request with photo
...
// add property called media_url in get request data
...
function uploadToSQL(req, profile_img, res) {
  ...
  return new Promise((resolve) => {
    const queryText = `INSERT INTO "client" ( user_id, client_name, home_address_house, apt_suite, city, state, zip_code, profile_img, about_client, about_home, about_equipment, contact_name_1, contact_phone_1, contact_email_1, vet_clinic, clinic_address, clinic_phone, transport)
...
function uploadPetProfileToSQL(req, profile_img, res) {
  return new Promise((resolve) => {
    ...
    pool
      .query(queryText, [id, profile_img])
      .then((result) => {
