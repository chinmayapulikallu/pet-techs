...
router.get("/picture/:id", (req, res) => {
  const sqlText = `SELECT * from pet_picture where pet_id = $1; `;
  pool
    .query(sqlText, [req.params.id])
    .then((response) => {
      console.log('pet picture:', response.rows)
      res.send(response.rows);
    })
    .catch((error) => {
      console.log(`Error making get pet pictures by id request. ${sqlText}`, error);
      res.sendStatus(500);
    });
});
